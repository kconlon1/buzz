...
export function observedUnreadStorageKey(
  ...
  return `${OBSERVED_UNREAD_STORAGE_PREFIX}:${normalizeRelayUrl(relayUrl)}:${pubkey}`;
}
 ...
 * Used when a channel is marked read (clearObserved path).
 */
export function removeChannelFromObservedUnreadStorage(
  ...
  try {
    ...
    const channels = eventsByChannelRaw as Record<string, unknown>;
    if (!Object.hasOwn(channels, channelId)) return;

    delete channels[channelId];
    const updated: PersistedPayload = {
      updatedAt: Date.now(),
      eventsByChannel: channels as Record<string, ObservedUnreadEvent[]>,
    };

    if (Object.keys(updated.eventsByChannel).length === 0) {
      window.localStorage.removeItem(key);
      return;
    }

    setLocalStorageItemWithRecovery(key, JSON.stringify(updated));
  } catch {
 ...
 * A pending timer for any scope is replaced. The timer closure captures an
 * immutable scope so a late A-scope timer can never overwrite B's bucket.
 */
export function scheduleObservedUnreadWrite(
  ...
  refs.timerRef.current = setTimeout(() => {
    ...
    writeObservedUnreadToStorage(pubkey, relayUrl, refs.eventsRef.current);
  }, 1_000);
}
