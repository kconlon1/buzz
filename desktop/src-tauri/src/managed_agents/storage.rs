...
///
/// Both restores are attempted independently on failure so a restore failure
/// in one store does not prevent the other from being restored. Errors from
/// both restores are aggregated into the returned error message (I5).
